...
#
# usage: Rscript 03-diffExp_highlowSBI.R
################################################################################
...
file_volc_hgat_plot <- file.path(analysis_dir, "plots", "enhancedVolcano_hgg_sbi.pdf")
file_tiff_volc_hgat_plot <- file.path(analysis_dir, "plots", "enhancedVolcano_hgg_sbi.tiff")
...
filtered.counts <- count_data[rowSums(count_data>=100) == 74, ]
countTable <- filtered.counts

## construct metadata
design = data.frame(row.names = colnames(countTable),
                    condition = c(rep("Low",46), rep("High",28)))
