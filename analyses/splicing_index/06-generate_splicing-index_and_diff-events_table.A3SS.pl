...
############################################################################################################
# 06-generate_splicing-index_and_diff-events_table.A3SS.pl
#
...
open(EVENTS,">results/splice_events.diff.A3SS.txt");
foreach my $sample(@bs_ids_uniq)
{
  foreach my $splice_event(@splicing_events_uniq)
  {
    ...
    if($psi_tumor > ($mean_psi + ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tA3SS\tSkipping\n";
    ...
    if($psi_tumor < ($mean_psi - ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tA3SS\tInclusion\n";
