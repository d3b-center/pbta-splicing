...
############################################################################################################
# 05-generate_splicing-index_and_diff-events_table.A5SS.pl
#
...
while(<FIL>)
{
  ...
  my $chr          = $cols[5];
  my $str          = $cols[6];
  my $exonStart    = $cols[19]+1; ## its 0-based so add 1
  my $exonEnd      = $cols[20];
  my $upstreamES   = $cols[21];
  my $upstreamEE   = $cols[22];
  my $downstreamES = $cols[23];
  my $downstreamEE = $cols[24];
...
open(EVENTS,">results/splice_events.diff.A5SS.txt");
...
foreach my $sample(@bs_ids_uniq)
{
  foreach my $splice_event(@splicing_events_uniq)
  {
    ...
    if($psi_tumor > ($mean_psi + ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tA5SS\tSkipping\n";
    ...
    if($psi_tumor < ($mean_psi - ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tA5SS\tInclusion\n";
...
open(TAB,">results/splicing_index.total.A5SS.txt");
