...
############################################################################################################
# 01-generate_splicing_index_tab_using_tumors.pl
#
...
############################################################################################################
my ($histology,$rmats_tsv,$primary_tumor_dat,$primary_tumor_plus_dat, $user_histology) = ($ARGV[0], $ARGV[1], $ARGV[2],$ARGV[3],$ARGV[4],$ARGV[5]);
...
# annotate histology file #
  ...
  open(FIL,$histology) || die("Cannot Open File");
    while(<FIL>)
    {
      ...
      next unless ( $hist=~/$user_histology/);
...
while(<FIL>)
{
  ...
  ## skip if histologies dont match user inputted histology
...
open(EVENTS,">results/splice_events.diff.SE.txt");
foreach my $sample(@bs_ids_uniq)
{
  foreach my $splice_event(@splicing_events_uniq)
  {
    ...
    if($psi_tumor > ($mean_psi + ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tSE\tSkipping\n";
    }
    # < -2 z-scores
    if($psi_tumor < ($mean_psi - ($std_psi + $std_psi)) )
    {
      $absplice_totals_per_sample_neg{$sample}++;
      print EVENTS $splice_event,"\tSE\tInclusion\n";
    }
  }
}
close(EVENTS);
...
open(TAB,">results/splicing_index.total.".$user_histology.".txt");
