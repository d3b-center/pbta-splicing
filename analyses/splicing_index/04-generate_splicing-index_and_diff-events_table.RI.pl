...
############################################################################################################
# 04-generate_splicing-index_and_diff-events_table.RI.pl 
#
...
while(<FIL>)
{
  ...
  my $chr          = $cols[5];
  my $str          = $cols[6];
  my $exonStart    = $cols[13]+1; ## its 0-based so add 1
  my $exonEnd      = $cols[14];
...
open(EVENTS,">results/splice_events.diff.RI.txt");
foreach my $sample(@bs_ids_uniq)
{
  foreach my $splice_event(@splicing_events_uniq)
  {
    ...
    if($psi_tumor > ($mean_psi + ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tRI\tSkipping\n";
    ...
    if($psi_tumor < ($mean_psi - ($std_psi + $std_psi)) )
    {
      ...
      print EVENTS $splice_event,"\tRI\tInclusion\n";
...
open(TAB,">results/splicing_index.RI.total.txt");
