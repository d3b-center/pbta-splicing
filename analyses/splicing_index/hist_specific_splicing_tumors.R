...
## exon skipping
ATRT_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.ATRT.txt"), sep = "\t", header=FALSE)
CPG_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.Craniopharyngioma.txt"), sep = "\t", header=FALSE)
GNG_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.Ganglioglioma.txt"), sep = "\t", header=FALSE)
EPN_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.Ependymoma.txt"), sep = "\t", header=FALSE)
HGAT_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.HGAT.txt"), sep = "\t", header=FALSE)
MB_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.Medulloblastoma.txt"), sep = "\t", header=FALSE)
LGAT_events = read.delim(paste0(analysis_dir, "splicing_events.hist-labeled_list.thr10freq.pos.LGAT.txt"), sep = "\t", header=FALSE)
...
es_events <- as.grob(upset(fromList(listInput), 
...
ggsave(paste0(plots_dir, "upsetR_histology-specific.pdf"), width = 16, height = 4)
