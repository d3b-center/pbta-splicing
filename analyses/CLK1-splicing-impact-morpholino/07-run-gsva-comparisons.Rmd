...
score_names <- c("kegg", "hallmark", "dna_repair", "kegg_de", "hallmark_de", "dna_repair_de") # Names for output files
...
for (i in 1:length(score_sources)) {
  ...
  score_df_summarized <- score_df %>%
    ...
    summarise(Mean_GSVA_Score = mean(gsva_score, na.rm = TRUE)) %>%
    ungroup() 
...
heatmap_obj <- Heatmap(
  gsva_scores_mat,
  col = colorRamp2(c(-.8, 0, .8), c("blue", "white", "darkorange")),
  name = "GSVA Scores",
  na_col = "whitesmoke",
