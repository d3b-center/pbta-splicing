...
# Add histology columns to splice index, and code PFS_status using following logic
splice_index <- splice_index %>%
  left_join(hist[,c("Kids_First_Biospecimen_ID", "cohort_participant_id", 
                    "molecular_subtype", "OS_days", "OS_status", "PFS_days",
                    "extent_of_tumor_resection")]) %>%
  mutate(PFS_days = as.numeric(PFS_days)) %>%
  mutate(PFS_years = PFS_days / 365.25,
