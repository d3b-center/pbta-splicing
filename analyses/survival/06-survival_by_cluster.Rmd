...
```{r}
km_os_plot <- plotKM(model = lgg_kap_os,
                    variable = "cluster_assigned",
                    combined = F, 
                    title = "cluster")
...
km_efs_plot <- plotKM(model = lgg_kap_efs,
                    variable = "cluster_assigned",
                    combined = F, 
                    title = "cluster")

ggsave(file.path(plot_dir, "km_OS_LGG_cluster_assignment.pdf"),
       width = 10, height = 6, units = "in",
       device = "pdf")
...
add_model_lgg_efs <- fit_save_model(lgg[!lgg$cluster_assigned %in% c(2,5,8,11),],
                              terms = "extent_of_tumor_resection+mol_sub_group+cluster_assigned",
...
```{r}
hgg <- metadata %>%
  dplyr::filter(Histology == "Other high-grade glioma") %>%
