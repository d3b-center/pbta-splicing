...
for (group in groups){
  ...
  add_model_os <- suppressWarnings(fit_save_model(group_hist,
                                  terms = ifelse(
                                      group %in% c("Mixed neuronal-glial tumor", "Low-grade glioma"), "mol_sub_group+extent_of_tumor_resection+SI_group",
                                      ifelse(group %in% c("Atypical Teratoid Rhabdoid Tumor", "Ependymoma", "DIPG or DMG", "Other high-grade glioma", "Medulloblastoma"), "mol_sub_group+SI_group", "SI_group")
                                      ),
             coxph_os_model_file,
             "multivariate",
             years_col = "OS_years",
...
subtype_df <- metadata %>%
  group_by(Histology, molecular_subtype, mol_sub_group, OS_status) %>%
  filter(!is.na(mol_sub_group),
         Histology %in% groups) %>%
  count() %>%
  filter(n >=3) %>%
  dplyr::rename(hist = Histology,
                subtype = mol_sub_group) %>%
  ...
  mutate(subtype = gsub("^HGG, |^ATRT, |CRANIO, |DHG, H3 |EPN, |IHG, |MB, |, TP53|DMG, H3 | alteration", "", subtype))
