...
mixed_events_df <- inner_join(psi_incl,psi_skip, by="Splice ID") %>%
  ...
  dplyr::rename("unidirectional_skip_events_df"="Splice ID")
...
unidirectional_skip_events_func_df <-  unidirectional_skip_events_df %>% 
  filter(!unidirectional_skip_events_df %in% mixed_events_df$SpliceID) %>% 
