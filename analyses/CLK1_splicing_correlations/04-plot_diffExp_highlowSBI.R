...
#
# usage: Rscript 04-plot-diffExp_highlowSBI.R
################################################################################
...
counts_combined <- inner_join(count_data_SF_high, count_data_SF_low, by = 'gene')  %>%
  rowwise %>% 
  dplyr::filter(sum(c_across(where(is.numeric))) >= 100) %>%
  ungroup
...
design = data.frame(row.names = colnames(filtered_counts_gene_rm),
                    condition = c(rep("High",14), rep("Low",15)))
...
res <- as_tibble(res) %>% add_column(counts_combined$gene) 
