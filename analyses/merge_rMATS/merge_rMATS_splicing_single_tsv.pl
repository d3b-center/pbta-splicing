...
my @SE_dir=<~/Desktop/pan_cancer_rmats/*SE.MATS.JC.txt>;
...
## ignore for now
# ## parse config file and store file information
# open(CONF, $config_file) || ("Cannot Open File");
# while(<CONF>)
# {
#   chomp;
#   if($_=~/hist/)
#   {
#     ($field, $histology) = split "\=";
#   }
#   elsif($_=~/SE/)
#   {
#     ($field, $SE_dir) = split "\=";
...
#   elsif($_=~/A5SS/)
#   {
#     ($field, $A5SS_dir) = split "\=";
#   }
#   elsif($_=~/A3SS/)
#   {
#     ($field, $A3SS_dir) = split "\=";
#   }
#   else{
#     ($field, $RI_dir) = split "\=";
#   }
# }
## go through each tsv in the dirs
#print $config_file,"\n";
#print $SE_dir,"\n",$MXE_dir,"\n",$A5SS_dir,"\n",$A3SS_dir,"\n",$RI_dir,"\n";
    ...
    if($_=~/^ID/)
    {
      ...
      foreach my $full_header(@full_header)
      {
        ...
        if(   ($full_header eq "1stExonStart_0base") ||($full_header eq "1stExonEnd") || ($full_header eq "2ndExonStart_0base") || ($full_header eq "2ndExonEnd")
            ...
            || ($full_header eq "IJC_SAMPLE_2")  || ($full_header eq "SJC_SAMPLE_2") || ($full_header eq "IncLevel2")  || ($full_header eq "IncLevelDifference"))
            {
              print "NA\t";
              next;
            }
