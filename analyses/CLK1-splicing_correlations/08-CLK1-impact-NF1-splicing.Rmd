...
median_data <- plot_df %>%
  filter(event_type == "CLK1-Exon4_PSI") %>%
  group_by(plot_group) %>%
...
# Reorder plot_group levels based on the sorted median IncLevel1
plot_df$event_type <- factor(plot_df$event_type, levels = c("CLK1-Exon4_PSI", "NF1-Exon23a_PSI", "NF1-215_PSI", "NF1-208_PSI"))

# Check that we have an equal number of samples across all events
...
subset_tpm_df <- transcript_tpm_df %>%
  as.data.frame() %>%
  select(`Total CLK1`, `CLK1-201`, `CLK1-Exon4_PSI`, `Total NF1`, `NF1-202_PC`, `NF1-208_PSI`, `NF1-208_NMD`, `NF1-215_PSI`, `NF1-215_RI`,
         `NF1-Exon23a_PSI`) %>%
  as.matrix()

# Compute the correlation matrix
mat_subset <- cor(as.matrix(subset_tpm_df))

items_of_interest <- c("Total CLK1", "CLK1-201", "CLK1-Exon4_PSI", "Total NF1", "NF1-202_PC", "NF1-208_PSI", "NF1-208_NMD", "NF1-215_PSI", "NF1-215_RI",
         "NF1-Exon23a_PSI")
...
for (group in plot_groups) {
  ...
  # Compute the correlation matrix
  gp_mat <- cor(as.matrix(subset_group, use = "pairwise.complete.obs"))
