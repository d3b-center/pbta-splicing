...
reformat <- transcript_tpm_df %>%
  ...
  dplyr::rename(`CLK1-201 (Exon4 Incl) PSI` = `CLK1-Exon4_PSI`,
                `NF1-202` = `NF1-202_PC`,
                `NF1-215 PSI` = `NF1-215_PSI`,
                `NF1-215` = `NF1-215_RI`,
                `NF1-202 (Exon23a Incl) PSI` =  `NF1-Exon23a_PSI`) %>%
  write_rds(file.path(results_dir, "clk1-nf1-psi-exp-phos-df.rds")) 

all_mat <- as.matrix(reformat)
...
# for HGG
items_of_interest_hgg <- c("Total CLK1", "CLK1-201", "CLK1-201 (Exon4 Incl) PSI", "Total NF1", "NF1-202", "NF1-215 PSI", "NF1-215",
         "NF1-202 (Exon23a Incl) PSI", "NF1 pS864", "NF1 pS2796", "Total NF1 Protein")

# for other 
items_of_interest_other <- c("Total CLK1", "CLK1-201", "CLK1-201 (Exon4 Incl) PSI", "Total NF1", "NF1-202", "NF1-215 PSI", "NF1-215",
         "NF1-202 (Exon23a Incl) PSI")


subset_tpm_df <- reformat %>%
  as.data.frame() %>%
...
for (group in plot_groups) {
  ...
  subset_group <- subset_tpm_df[bs_ids,to_subset]
  ...
  mtext("Spearman's Rho", side = 4, line = 1, cex = 1.2, font = 2, las = 0, padj = -3)
