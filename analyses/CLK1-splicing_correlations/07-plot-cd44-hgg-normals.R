...
gtex_plot_path <- file.path(plots_dir, "CD44-hgg-gtex.pdf")
...
# transcript is ENST00000263398.11 in pbta
hgg_counts <- readRDS(pbta_tpm_file) %>%
  dplyr::select(c(transcript_id, all_hgg_hist$Kids_First_Biospecimen_ID)) %>%
  filter(transcript_id == "ENST00000263398.11") %>%
  dplyr::select(-transcript_id) %>%
  # Transpose and then immediately convert to a data frame or tibble, capturing row names as a column
  {data.frame(t(.))} %>%
  rownames_to_column("Kids_First_Biospecimen_ID") %>%
  dplyr::rename(ENST00000263398 = `t...`) %>%
  left_join(all_hgg_hist[,c("Kids_First_Biospecimen_ID", "plot_group")])

# transcript is ENST00000263398.10 in gtex
gtex_clk1ex4_counts <- fread(gtex_trans_tpm_file, skip = 2) %>%
  dplyr::select(c(transcript_id, gtex_brain$Kids_First_Biospecimen_ID)) %>%
  filter(transcript_id == "ENST00000425428.6") %>%
  dplyr::select(-transcript_id) %>%
  {data.frame(t(.))} %>%
  rownames_to_column("Kids_First_Biospecimen_ID") %>%
  dplyr::rename(ENST00000263398 = `t...`) %>%
...
# plot
gtex_plot <- ggplot(combined_df_ordered, aes(x = plot_group, y = ENST00000263398)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(fill = plot_group), width = 0.2, size = 2, shape = 21, color = "black") + # Add actual data points
  scale_fill_manual(values = colors) +
  labs(title = "",
       x = "Tissue",
       y = expression(bold(bolditalic("CD44 ENST00000263398")*" TPM (log"[2]*")"))) +
  theme_Publication() +
