...
# Usage: Rscript 03-plot_CLK1-Ex4-splicing_vs_SRSF1-expr.R 
################################################################################
...
count_psi_midline_hgg_df <- filter(count_df,rownames(count_df) == 'SRSF1')  %>% 
...
## combine with psi values for scatter plot/correlation
count_psi_hgg_df <- filter(count_df,rownames(count_df) == 'SRSF1')  %>% 
...
plot_midline_hgg_path <- file.path(plots_dir, "SRSF1-expr_vs_psi-midlineHGG.tiff")
scatterplot_midline_hgg <- ggscatter(count_psi_midline_hgg_df, x="IncLevel2", y="Expr", 
                                     ...
                                     xlab = "SRSF11 Expr", ylab = "SRSF1 Expr") + theme_Publication()
...
plot_total_hgg_path <- file.path(plots_dir, "SRSF1-expr_vs_psi-totalHGG.tiff")
scatterplot_total_hgg <- ggscatter(count_psi_hgg_df, x="IncLevel2", y="Expr", 
                                   ...
                                   ticks = TRUE,
                                   #xticks.by = .1, yticks.by = .1,
                                   xlab = "Exon 4 PSI", ylab = "SRSF1 Expr") + theme_Publication()
