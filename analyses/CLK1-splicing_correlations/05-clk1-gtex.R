...
gtex_plot_path <- file.path(plots_dir, "CLK1ex4-gtex-hgg.pdf")
...
pal <- combined_df %>%
  ...
  mutate(plot_group_hex = case_when(is.na(plot_group_hex) ~ "lightgrey",
                                    TRUE ~ plot_group_hex))
...
gtex_plot <- ggplot(combined_df_ordered, aes(x = plot_group, y = ENST00000321356)) +
  ...
  geom_jitter(aes(fill = plot_group), width = 0.25, size = 2, shape = 21,color = "black") + # Add actual data points
...
pdf(gtex_plot_path, height = 6, width = 9, useDingbats = FALSE)
print(gtex_plot)
dev.off()
