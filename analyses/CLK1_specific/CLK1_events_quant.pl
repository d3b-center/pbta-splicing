...
####################################################################################################################
# CLK1_events_quant.pl
# descr: make table of sample ID, inclusion levels and histology
#
# usage: ./CLK1_events_quant.pl results/filtered_samples_vs_hist.dat results/CLK1_IncLevel_brain_tumors.tab.v2.txt
###################################################################################################################

my ($samples_hist, $clk1_tab) = ($ARGV[0], $ARGV[1]);
my %sample_hist;

## open and store subtype of samples
...
## print inclusion levels and associated histologies (for downstream plotting)
